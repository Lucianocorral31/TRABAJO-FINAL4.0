...
app.delete('/api/jobpostings/:id', async (req, res) => {
    try {
      ...
      if (!deletedPosting) {
        return res.status(404).json({ error: 'posting_not_found', message: 'La publicación no fue encontrada' });
      }
  
      res.json({ message: 'Publicación eliminada correctamente', deletedPosting });
    } catch (error) {
      console.error('Error al eliminar la publicación:', error);
      res.status(500).json({ error: 'server_error', message: 'Error al eliminar la publicación' });
    }
  });
